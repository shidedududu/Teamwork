...
public class TaskController {
    ...
    @ResponseBody
    public Map getTasks(@RequestBody GetTaskParam param){
        Map map=new HashMap();
        UserEntity us=userRepository.findByUserName(param.user.getUserName());
        ProjectEntity pro=projectRepository.findOne(param.project.getId());
        MemberEntity m=memberRepository.findByUserIdAndProId(us.getId(),param.project.getId());
        List<TaskEntity> taskList=new ArrayList<>();
       if(pro == null){
           map.put("result",0);
       }
       else if(pro.getUserByUserId().getId()==us.getId()){
           map.put("result",1);
    ...
    public Map addTask(@RequestBody TaskParam param){
        ...
        UserEntity us=userRepository.findByUserName(param.user.getUserName());
        ProjectEntity pro=projectRepository.findOne(param.project.getId());
        MemberEntity mem=memberRepository.findByUserIdAndProId(us.getId(),param.project.getId());
        param.task.setProjectByProId(pro);
        if(pro!=null){
            if((mem!=null && mem.getAuthority()==1) || pro.getUserByUserId().getId() == us.getId()){
